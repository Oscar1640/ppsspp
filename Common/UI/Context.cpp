...
void UIContext::BeginFrame() {
	...
	if (!uitexture_ || UIAtlas_ != lastUIAtlas_) {
		...
		if (!fontTexture_) {
			...
			// Don't bother with loading font_atlas_luna.zim
#else
			...
			if (!fontTexture_) {
				...
				fontTexture_ = CreateTextureFromFile(draw_, "asciifont_atlas_luna.zim", ImageFileType::ZIM, false);
				if (!fontTexture_) {
					WARN_LOG(SYSTEM, "Failed to load font_atlas_luna.zim or asciifont_atlas_luna.zim");
				}
			}
		}
	}
